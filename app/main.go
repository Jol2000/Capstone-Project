...
func main() {
	...
	homebtn := widget.NewButtonWithIcon("", theme.HomeIcon(), func() {
		...
		// Item label with border and larger number
		...
		go func() {
			for {
				...
				// Format the time string including seconds
		...
		// Container for date and time labels
		...
		// Button to navigate to collection list view
		...
		// Home icon toolbar next to welcome text
		...
		// Create a container for the settings and exit buttons and place them at the top-left corner
		topLeftButtons := container.NewHBox(settingbtn, exitbtn)

		// Layout the home icon toolbar and welcome text horizontally
		header := container.NewHBox(toolbar, overview, layout.NewSpacer())

		// Layout the labels and button in a modern design
		...
		// Place the overview container directly in the window
